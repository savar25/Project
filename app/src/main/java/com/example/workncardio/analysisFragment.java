...
public class analysisFragment extends Fragment {
    ...
    public  void init(){
        ...
        steps.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                    Calendar calendar=Calendar.getInstance();
                    for (int i = 0; i < 6; i++) {
                        String date = String.valueOf(calendar.get(Calendar.DAY_OF_MONTH) - i) + "_" + String.valueOf(calendar.get(Calendar.MONTH)) + "_" + String.valueOf(calendar.get(Calendar.YEAR));
                        if(snapshot.child(date).getValue(DataStepsModel.class)!=null && !temp.contains(snapshot.child(date).getValue(DataStepsModel.class))) {
                            ...
                            Log.d(TAG, "onDataChangeANALYSIS: " + temp.size());
                            Log.d(TAG, "onDataChangeANALYSIS: "+temp.get(i).getSteps());
                            Log.d(TAG, "onDataChange: "+temp.get(i).getDateNum());
                        }
                    }
    ...
    public void setChart(ArrayList<DataStepsModel> temp){
